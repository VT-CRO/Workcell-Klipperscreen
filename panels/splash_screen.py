...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        if os.path.exists(eagle_path):
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(eagle_path, 160, 140)
        ...
        self.labels["text"].set_markup(
            f"<span size='xx-large' weight='bold'>{_('Initializing printer...')}</span>"
        )
    ...
    def update_text(self, text):
        self.labels["text"].set_markup(
            f"<span size='xx-large' weight='bold'>{text}</span>"
        )
        self.show_restart_buttons()
